...
(function() {
    ...
    // Global Error Boundary (Best Practice)
    const errorBoundary = {
        errors: [],
        maxErrors: 10,
        ...
        attemptRecovery(error, source) {
            try {
                ...
                log.warn('Recovery attempt failed:', recoveryError);
            }
        }
    };
    ...
    const Utils = {
        ...
        handleVideoScrollPause() {
            try {
                ...
                // Check each video for visibility
                this._cachedVideoElements.forEach(videoData => {
                    ...
                    // Get viewport dimensions with mobile-specific fallbacks
    ...
    const registerContactForm = () => {
        ...
        handleRecaptchaComplete() {
                        try {
                            ...
                            log.warn('reCAPTCHA complete handler error:', e);
                        }
        },
        ...
        handleRecaptchaExpired() {
                        try {
                            ...
                            if (!this.isSubmitting) {
                                this.errorMessage = "reCAPTCHA verification expired. Please verify again.";
        ...
        async submitForm(event) {
            try {
                ...
                const recaptchaResponse = formData.get("g-recaptcha-response");
                            const recaptchaElement = Utils.safeQuery(".g-recaptcha", form) || Utils.safeQuery(".g-recaptcha");
                ...
                if (recaptchaElement) {
                    ...
                    if (!recaptchaResponse || !this.recaptchaCompleted) {
                        ...
                        this.recaptchaError = true;
                        this.clearErrorAfterDelay();
                                if (Utils.isMobile() && recaptchaElement) {
                                    Utils.safeScrollIntoView(recaptchaElement);
                    }
                        this._resetSubmissionState();
                        return;
                    }
                    // Clear reCAPTCHA error if valid
                    this.recaptchaError = false;
                }
            ...
            try {
                ...
                if (typeof grecaptcha !== "undefined") {
                    try {
                        ...
                        log.warn('reCAPTCHA reset error:', e);
                    }
                }
            } catch (e) {
                log.warn('Error resetting reCAPTCHA:', e);
            }
        },
            }));
            ...
            if (window.Alpine && Alpine.store) {
                Alpine.store('errors', {
                    ...
                    add(error) {
                        ...
                        setTimeout(() => {
                            this.list = this.list.filter(e => e.id !== this.list[this.list.length - 1].id);
                        }, 5000);
                    }
                });
            }
    ...
    window.onRecaptchaSuccess = function() {
        try {
            ...
            log.warn('reCAPTCHA success handler error:', e);
        }
    };
        ...
        try {
            ...
            forms.forEach(form => {
                ...
                if (typeof Alpine !== 'undefined' && form._x_dataStack && form._x_dataStack[0]) {
                    ...
                    if (formData && typeof formData.handleRecaptchaExpired === 'function') {
    ...
    window.onRecaptchaError = function() {
        try {
            ...
            forms.forEach(form => {
                ...
                if (typeof Alpine !== 'undefined' && form._x_dataStack && form._x_dataStack[0]) {
                    const formData = form._x_dataStack[0];
                    if (formData && typeof formData.handleRecaptchaError === 'function') {
    ...
    const initDropdownHandlers = () => {
        try {
            ...
            setTimeout(() => { forceRemoveCloak(); }, 100);
            setTimeout(() => { forceRemoveCloak(); }, 500);
            setTimeout(() => { forceRemoveCloak(); }, 1000);
            ...
            if (moreButton && moreDropdown) {
                ...
                const showDropdown = () => {
                    ...
                    if (!moreDropdown.dataset.originalParent) {
                        moreDropdown.dataset.originalParent = moreDropdown.parentElement.id || 'more-menu-container';
                    }
                    ...
                    requestAnimationFrame(() => {
                        // Just verify the dropdown is visible - don't touch position!
                        ...
                        // CRITICAL: Check if Alpine is still hiding it
                        if (styles.display === 'none' || styles.visibility === 'hidden' || parseFloat(styles.opacity) === 0 || rect.width === 0) {
                            console.error('❌ [DROPDOWN] STILL HIDDEN! Forcing visibility...');
                            
                            // Remove Alpine's x-show attribute entirely - this is the key!
                            moreDropdown.removeAttribute('x-show');
                            ...
                            // Recalculate position correctly
                            ...
                            // Force all styles again with even more aggression
                            moreDropdown.style.cssText = `
                                display: block !important;
                                visibility: visible !important;
                                opacity: 1 !important;
                            ...
                            setTimeout(() => {
                                const newRect = moreDropdown.getBoundingClientRect();
                                const newStyles = window.getComputedStyle(moreDropdown);
                            }, 50);
                        } else {
                            
                            // Double-check: Is it actually in the viewport?
                            ...
                            if (!isInViewport) {
                                // Dropdown is outside viewport
                            }
                            
                            // Check if it's actually visible (has dimensions)
                            if (rect.width === 0 || rect.height === 0) {
                                // Force it visible one more time
                ...
                console.error('❌ [DROPDOWN] Missing elements! Button:', !!moreButton, 'Dropdown:', !!moreDropdown);
            }
            ...
            if (mobileButton && mobileMenu) {
                ...
                const showMobileMenu = () => {
                    ...
                    if (!mobileMenu.dataset.originalParent) {
                        mobileMenu.dataset.originalParent = mobileMenu.parentElement.id || 'main-nav';
                    }
                ...
                console.error('❌ [MOBILE] Missing elements! Button:', !!mobileButton, 'Menu:', !!mobileMenu);
            }
        } catch (e) {
    ...
    const initFAQHandlers = () => {
        ...
        try {
            ...
            console.error('❌ [FAQ] Error in FAQ handler:', e);
        }
    };
